...
serve(async (req) => {
  ...
  try {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // ACTION: request_revision (Revizyon İsteme)
    // ─────────────────────────────────────────────────────────────────────────
    if (action === 'request_revision') {
      ...
      if (profile.role !== 'admin') {
        ...
        // Mentor bu teslimin takımından mı sorumlu?
        let isAuthorized = false

        // A) Teslim doğrudan takım id taşıyorsa
        if (teslim.takim_id) {
        ...
        // B) Katılımcı üzerinden takım kontrolü
